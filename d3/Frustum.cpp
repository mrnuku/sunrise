...
============
*/
void BoxToPoints( const Vec3 &center, const Vec3 &extents, const Mat3 &axis, Vec3 points[8] ) {
	Mat3 ax;
	Vec3 temp[4];
...
bool Frustum::CullPoint( const Vec3 &point ) const {
	...
	scale = p.x * invFar;
	if ( fabs( p.y ) > dLeft * scale ) {
		return true;
	}
	if ( fabs( p.z ) > dUp * scale ) {
		return true;
	}
	return false;
}
...
============
*/
bool Frustum::CullLocalBox( const Vec3 &localOrigin, const Vec3 &extents, const Mat3 &localAxis ) const {
	float d1, d2;
	Vec3 testOrigin;
	Mat3 testAxis;
	...
	d2 = fabs( extents[0] * localAxis[0][0] ) +
				fabs( extents[1] * localAxis[1][0] ) +
						fabs( extents[2] * localAxis[2][0] );
	if ( d1 - d2 > 0.0f ) {
		return true;
	}
	...
	d2 = fabs( extents[0] * ( dFar * testAxis[0][1] - dLeft * testAxis[0][0] ) ) +
				fabs( extents[1] * ( dFar * testAxis[1][1] - dLeft * testAxis[1][0] ) ) +
					fabs( extents[2] * ( dFar * testAxis[2][1] - dLeft * testAxis[2][0] ) );
	if ( d1 - d2 > 0.0f ) {
		return true;
	}
	...
	d2 = fabs( extents[0] * ( dFar * testAxis[0][2] - dUp * testAxis[0][0] ) ) +
				fabs( extents[1] * ( dFar * testAxis[1][2] - dUp * testAxis[1][0] ) ) +
					fabs( extents[2] * ( dFar * testAxis[2][2] - dUp * testAxis[2][0] ) );
	if ( d1 - d2 > 0.0f ) {
		return true;
	}
...
============
*/
bool Frustum::CullBounds( const Bounds &bounds ) const {
...
============
*/
bool Frustum::CullBox( const Box &box ) const {
	Vec3 localOrigin;
	Mat3 localAxis;
...
bool Frustum::CullSphere( const Sphere &sphere ) const {
	...
	// test left/right planes
	d = dFar * fabs( center.y ) - dLeft * center.x;
	if ( ( d * d ) > rs * ( sFar + dLeft * dLeft ) ) {
		return true;
	}

	// test up/down planes
	d = dFar * fabs( center.z ) - dUp * center.x;
...
bool Frustum::CullLocalWinding( const Vec3 *points, const int numPoints, int *pointCull ) const {
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( fabs( p.y ) > p.x * leftScale ) {
			pCull |= 4 << FLOATSIGNBITSET( p.y );
		}
		if ( fabs( p.z ) > p.x * upScale ) {
...
============
*/
bool Frustum::CullWinding( const Winding &winding ) const {
	int i, *pointCull;
	Vec3 *localPoints;
	Mat3 transpose;

	localPoints = (Vec3 *) _alloca16( winding.GetNumPoints() * sizeof( Vec3 ) );
...
/*
============
Frustum::BoundsCullLocalFrustum

  Tests if any of the bounding box planes can be used as a separating plane.
============
*/
bool Frustum::BoundsCullLocalFrustum( const Bounds &bounds, const Frustum &localFrustum, const Vec3 indexPoints[8], const Vec3 cornerVecs[4] ) const {
	int index;
	float dx, dy, dz;
...
bool Frustum::LocalLineIntersection( const Vec3 &start, const Vec3 &end ) const {
	...
	if ( dNear > 0.0f ) {
		...
		if ( FLOATNOTZERO( d1 ) ) {
			...
			if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
				f = d1 / ( d1 - d2 );
				if ( fabs( start.y + f * dir.y ) <= dNear * leftScale ) {
					if ( fabs( start.z + f * dir.z ) <= dNear * upScale ) {
						return true;
					}
				}
			}
	...
	if ( FLOATNOTZERO( d1 ) ) {
		...
		if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
			f = d1 / ( d1 - d2 );
			if ( fabs( start.y + f * dir.y ) <= dFar * leftScale ) {
				if ( fabs( start.z + f * dir.z ) <= dFar * upScale ) {
					return true;
				}
			}
		}
	...
	if ( FLOATNOTZERO( d1 ) ) {
		...
		if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
			...
			x = start.x + f * dir.x;
			if ( x >= dNear && x <= dFar ) {
				if ( fabs( start.z + f * dir.z ) <= x * upScale ) {
					return true;
				}
			}
		}
	...
	if ( FLOATNOTZERO( d1 ) ) {
		...
		if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
			...
			x = start.x + f * dir.x;
			if ( x >= dNear && x <= dFar ) {
				if ( fabs( start.y + f * dir.y ) <= x * leftScale ) {
					return true;
				}
			}
		}
...
/*
============
Frustum::LocalFrustumIntersectsFrustum
============
*/
...
/*
============
Frustum::LocalFrustumIntersectsBounds
============
*/
bool Frustum::LocalFrustumIntersectsBounds( const Vec3 points[8], const Bounds &bounds ) const {
	int i;
...
============
*/
bool Frustum::IntersectsBounds( const Bounds &bounds ) const {
	Vec3 localOrigin, center, extents;
	Mat3 localAxis;
...
bool Frustum::IntersectsBox( const Box &box ) const {
	...
	if ( BoundsCullLocalFrustum( Bounds( -box.GetExtents(), box.GetExtents() ), localFrustum, indexPoints, cornerVecs ) ) {
		return false;
	}
	...
	if ( LocalFrustumIntersectsBounds( indexPoints, Bounds( -box.GetExtents(), box.GetExtents() ) ) ) {
		return true;
	}
...
bool Frustum::IntersectsSphere( const Sphere &sphere ) const {
	...
	if ( p.x <= dNear ) {
		scale = dNear * invFar;
		dir.y = fabs( p.y ) - dLeft * scale;
		dir.z = fabs( p.z ) - dUp * scale;
	}
	else if ( p.x >= dFar ) {
		dir.y = fabs( p.y ) - dLeft;
		dir.z = fabs( p.z ) - dUp;
	}
	else {
		scale = p.x * invFar;
...
/*
============
Frustum::IntersectsFrustum
============
*/
bool Frustum::IntersectsFrustum( const Frustum &frustum ) const {
...
/*
============
Frustum::IntersectsWinding
============
*/
bool Frustum::IntersectsWinding( const Winding &winding ) const {
	int i, j, *pointCull;
	float min, max;
	Vec3 *localPoints, indexPoints[8], cornerVecs[4];
	Mat3 transpose;
	Plane plane;
...
============
*/
bool Frustum::FromProjection( const Bounds &bounds, const Vec3 &projectionOrigin, const float dFar ) {
	return FromProjection( Box( bounds, vec3_origin, mat3_identity ), projectionOrigin, dFar );
}
...
============
*/
bool Frustum::FromProjection( const Box &box, const Vec3 &projectionOrigin, const float dFar ) {
	int i, bestAxis;
	...
	bestAxis = 0;
	bestValue = fabs( box.GetAxis()[0] * dir );
	for ( i = 1; i < 3; i++ ) {
		value = fabs( box.GetAxis()[i] * dir );
	...
	for ( j = 0; j < 2; j++ ) {
		...
		if ( j == 0 ) {
			dir += Tan16( 0.5f * ( ATan16( points[minY].y, points[minY].x ) + ATan16( points[maxY].y, points[maxY].x ) ) ) * axis[1];
			dir += Tan16( 0.5f * ( ATan16( points[minZ].z, points[minZ].x ) + ATan16( points[maxZ].z, points[maxZ].x ) ) ) * axis[2];
			dir.Normalize();
		}
	}
	...
	this->dFar = dFar;
	this->dLeft = Max( fabs( points[minY].y / points[minY].x ), fabs( points[maxY].y / points[maxY].x ) ) * dFar;
	this->dUp = Max( fabs( points[minZ].z / points[minZ].x ), fabs( points[maxZ].z / points[maxZ].x ) ) * dFar;
	...
	for ( j = 0; j < 2; j++ ) {
		...
		if ( j == 0 ) {
			dir += Tan16( 0.5f * ( ATan16( b[1][1] ) + ATan16( b[0][1] ) ) ) * axis[1];
			dir += Tan16( 0.5f * ( ATan16( b[1][2] ) + ATan16( b[0][2] ) ) ) * axis[2];
			dir.Normalize();
		}
	}
	...
	this->dNear = b[0][0];
	this->dFar = dFar;
	this->dLeft = Max( fabs( b[0][1] ), fabs( b[1][1] ) ) * dFar;
	this->dUp = Max( fabs( b[0][2] ), fabs( b[1][2] ) ) * dFar;
	this->invFar = 1.0f / dFar;
	...
	for ( i = 0; i < 3; i++ ) {
		dist[i] = fabs( box.GetExtents()[0] * ( axis[i] * box.GetAxis()[0] ) ) +
					fabs( box.GetExtents()[1] * ( axis[i] * box.GetAxis()[1] ) ) +
						fabs( box.GetExtents()[2] * ( axis[i] * box.GetAxis()[2] ) );
	}
...
/*
============
Frustum::ConstrainToBounds

  Returns false if no part of the bounds extends beyond the near plane.
============
*/
bool Frustum::ConstrainToBounds( const Bounds &bounds ) {
...
============
*/
bool Frustum::ConstrainToBox( const Box &box ) {
	float min, max, newdFar;
...
/*
============
Frustum::ToIndexPointsAndCornerVecs
...
============
*/
void Frustum::AxisProjection( const Mat3 &ax, Bounds &bounds ) const {
	Vec3 indexPoints[8], cornerVecs[4];
...
/*
============
Frustum::AddLocalLineToProjectionBoundsSetCull
============
*/
void Frustum::AddLocalLineToProjectionBoundsSetCull( const Vec3 &start, const Vec3 &end, int &startCull, int &endCull, Bounds &bounds ) const {
	Vec3 dir, p;
...
/*
============
Frustum::AddLocalLineToProjectionBoundsUseCull
============
*/
void Frustum::AddLocalLineToProjectionBoundsUseCull( const Vec3 &start, const Vec3 &end, int startCull, int endCull, Bounds &bounds ) const {
	Vec3 dir, p;
	float d1, d2, fstart, fend, lstart, lend, f;
	float leftScale, upScale;
	int clip;
...
/*
============
Frustum::BoundsRayIntersection

  Returns true if the ray starts inside the bounds.
  If there was an intersection scale1 <= scale2
============
*/
bool Frustum::BoundsRayIntersection( const Bounds &bounds, const Vec3 &start, const Vec3 &dir, float &scale1, float &scale2 ) const {
	Vec3 end, p;
	float d1, d2, f;
	int i, startInside = 1;
...
/*
============
Frustum::ProjectionBounds
============
*/
bool Frustum::ProjectionBounds( const Bounds &bounds, Bounds &projectionBounds ) const {
	return ProjectionBounds( Box( bounds, vec3_origin, mat3_identity ), projectionBounds );
}
...
============
*/
bool Frustum::ProjectionBounds( const Box &box, Bounds &projectionBounds ) const {
	int i, p1, p2, pointCull[8], culled, outside;
	float scale1, scale2;
	Frustum localFrustum;
	Vec3 points[8], localOrigin;
	Mat3 localAxis, localScaled;
	Bounds bounds( -box.GetExtents(), box.GetExtents() );
...
============
*/
bool Frustum::ProjectionBounds( const Sphere &sphere, Bounds &projectionBounds ) const {
	float d, r, rs, sFar;
	Vec3 center;
...
============
*/
bool Frustum::ProjectionBounds( const Frustum &frustum, Bounds &projectionBounds ) const {
	int i, p1, p2, pointCull[8], culled, outside;
	float scale1, scale2;
	Frustum localFrustum;
	Vec3 points[8], localOrigin;
...
============
*/
bool Frustum::ProjectionBounds( const Winding &winding, Bounds &projectionBounds ) const {
	int i, p1, p2, *pointCull, culled, outside;
	float scale;
	Vec3 *localPoints;
	Mat3 transpose, scaled;
	Plane plane;
	...
	if ( outside != 1 && outside != 2 && outside != 4 && outside != 8 ) {
		...
		if ( (outside & 2) && (outside & 8) ) {
			if ( winding.RayIntersection( plane, origin, scaled[0] - scaled[1] - scaled[2], scale ) ) {
				projectionBounds.AddPoint( Vec3( scale * dFar, -1.0f, -1.0f ) );
			}
		}
		if ( (outside & 2) && (outside & 4) ) {
			if ( winding.RayIntersection( plane, origin, scaled[0] - scaled[1] + scaled[2], scale ) ) {
...
============
*/
void Frustum::ClipFrustumToBox( const Box &box, float clipFractions[4], int clipPlanes[4] ) const {
	int i, index;
	float f, minf;
	Mat3 scaled, localAxis, transpose;
	Vec3 localOrigin, cornerVecs[4];
	Bounds bounds;
...
bool Frustum::ClipLine( const Vec3 localPoints[8], const Vec3 points[8], int startIndex, int endIndex, Vec3 &start, Vec3 &end, int &startClip, int &endClip ) const {
	...
	Vec3 localStart, localEnd, localDir;
	...
	if ( FLOATNOTZERO( d1 ) ) {
		if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
			...
			if ( x >= 0.0f ) {
				if ( fabs( localStart.z + f * localDir.z ) <= x * upScale ) {
	...
	if ( FLOATNOTZERO( d1 ) ) {
		if ( FLOATSIGNBITSET( d1 ) ^ FLOATSIGNBITSET( d2 ) ) {
			...
			x = localStart.x + f * localDir.x;
			if ( x >= 0.0f ) {
				if ( fabs( localStart.y + f * localDir.y ) <= x * leftScale ) {
...
/*
============
Frustum::AddLocalCapsToProjectionBounds
============
*/
...
INLINE bool Frustum::AddLocalCapsToProjectionBounds( const Vec3 endPoints[4], const int endPointCull[4], const Vec3 &point, int pointCull, int pointClip, Bounds &projectionBounds ) const {
	int *p;
...
/*
============
Frustum::ClippedProjectionBounds
============
*/
bool Frustum::ClippedProjectionBounds( const Frustum &frustum, const Box &clipBox, Bounds &projectionBounds ) const {
	...
	Mat3 localAxis1, localAxis2, transpose;
	Bounds clipBounds;
