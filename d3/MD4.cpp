...
is entified as the "RSA Data Security, Inc. MD4 Message-Digest
Algorithm" in all material mentioning or referencing this software
or this function.

License is also granted to make and use derivative works provided
that such works are entified as "derived from the RSA Data
...
static void MD4_Transform( UINT4 state[ 4 ], const unsigned char block[ 64 ] ) {
	...
	Common::Com_Memset( ( POINTER )x, 0, sizeof( x ) );
}
...
void MD4_Final( MD4_CTX * context, unsigned char digest[ 16 ] ) {
	...
	/* Zeroize sensitive information.*/
	Common::Com_Memset( ( POINTER )context, 0, sizeof( *context ) );
}

unsigned Common::Com_BlockChecksum( const void * buffer, size_t length ) {
	...
	MD4_Init( &ctx );
	MD4_Update( &ctx, ( unsigned char * )buffer, ( int )length );
