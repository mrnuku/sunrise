...
/*
=============
Winding::BaseForPlane
=============
*/
void Winding::BaseForPlane( const Vec3 &normal, const float dist ) {
	Vec3 org, vright, vup;
...
=============
*/
Winding *Winding::Clip( const Plane &plane, const float epsilon, const bool keepOn ) {
	float *		dists;
	byte *		sides;
	Vec5 *	newPoints;
	int			newNumPoints;
	...
	newPoints = (Vec5 *) _alloca16( maxpts * sizeof( Vec5 ) );
	newNumPoints = 0;
	...
	memcpy( p, newPoints, newNumPoints * sizeof(Vec5) );
...
=============
*/
Winding *Winding::Copy( void ) const {
	Winding *w;
...
=============
*/
Winding *Winding::Reverse( void ) const {
	Winding *w;
	int i;
...
=============
*/
Winding & Winding::ReverseSelf( void ) {
	Vec5 v;
	int i;
...
bool Winding::Check( bool print ) const {
	...
	if ( numPoints < 3 ) {
		if ( print ) {
			Common::Com_Printf( "Winding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if ( area < 1.0f ) {
		if ( print ) {
			Common::Com_Printf( "Winding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		for ( j = 0; j < 3; j++ ) {
			if ( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD ) {
				if ( print ) {
					Common::Com_Printf( "Winding::Check: point %d outside world %c-axis: %f", i, 'X'+j, p1[j] );
				}
				return false;
			}
		}
		...
		if ( d < -ON_EPSILON || d > ON_EPSILON ) {
			if ( print ) {
				Common::Com_Printf( "Winding::Check: point %d off plane.", i );
			}
			return false;
		}
		...
		if ( dir.Length() < ON_EPSILON) {
			if ( print ) {
				Common::Com_Printf( "Winding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for ( j = 0; j < numPoints; j++ ) {
			...
			if ( d > edgedist ) {
				if ( print ) {
					Common::Com_Printf( "Winding::Check: non-convex." );
				}
				return false;
			}
		}
...
=============
*/
float Winding::GetArea( void ) const {
	int i;
	Vec3 d1, d2, cross;
	float total;
...
=============
*/
Vec3 Winding::GetCenter( void ) const {
	int i;
	Vec3 center;
...
/*
=============
Winding::InsertPointIfOnEdge
=============
*/
bool Winding::InsertPointIfOnEdge( const Vec3 &point, const Plane &plane, const float epsilon ) {
	int i;
	float dist, dot;
	Vec3 normal;
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( fabs( normal * point - dist ) > epsilon ) {
			continue;
		}
...
bool Winding::IsTiny( void ) const {
	int		i;
	float	len;
	Vec3	delta;
	int		edges;
...
=============
*/
bool Winding::IsHuge( void ) const {
	int i, j;
...
=============
*/
void Winding::Print( void ) const {
	int i;

	for ( i = 0; i < numPoints; i++ ) {
		Common::Com_Printf( "(%5.1f, %5.1f, %5.1f)\n", p[i][0], p[i][1], p[i][2] );
	}
}

/*
=============
Winding::PlaneDistance
=============
*/
...
/*
=================
Winding::TriangleArea
=================
*/
float Winding::TriangleArea( const Vec3 &a, const Vec3 &b, const Vec3 &c ) {
	Vec3	v1, v2;
	Vec3	cross;
...
bool FixedWinding::ReAllocate( int n, bool keep ) {
	...
	if ( n > MAX_POINTS_ON_WINDING ) {
		Common::Com_Printf("WARNING: FixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
		return false;
	}
	return true;
}
...
=============
*/
int FixedWinding::Split( FixedWinding *back, const Plane &plane, const float epsilon ) {
	int		counts[3];
