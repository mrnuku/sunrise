...
void Blip_Buffer::clear( int entire_buffer )
{
	...
	if ( buffer_ )
	{
		...
		Common::Com_Memset( buffer_, 0, (count + buffer_extra) * sizeof (buf_t_) );
	}
}
...
Blip_Buffer::blargg_err_t Blip_Buffer::set_sample_rate( long new_rate, int msec )
{
	...
	{
		if( buffer_ ) Mem_Free( buffer_ );
		void* p = Mem_Alloc( (new_size + buffer_extra) * sizeof *buffer_ );
		if ( !p )
...
void Blip_Buffer::remove_samples( long count )
{
	if ( count )
	{
		...
		Common::Com_Memset( buffer_ + remain, 0, count * sizeof *buffer_ );
	}
}
